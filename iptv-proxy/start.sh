...
echo "[debug] xtreme base url: ${XTREME_BASE_URL}" | ts '%Y-%m-%d %H:%M:%.S'
echo "[debug] xtreme username: ${XTREME_USERNAME}" | ts '%Y-%m-%d %H:%M:%.S'
echo "[debug] xtreme password: ${XTREME_PASSWORD}" | ts '%Y-%m-%d %H:%M:%.S'
...
echo "[debug] Proxy command: 'iptv-proxy --xtreme-base-url ${XTREME_BASE_URL} --xtreme-user ${XTREME_USERNAME} --xtreme-password ${XTREME_PASSWORD} --user $PROXY_USER --password $PROXY_PASS --hostname $PROXY_HOST --custom-endpoint $PROXY_PATH &" | ts '%Y-%m-%d %H:%M:%.S'
echo "[info] Starting iptv-proxy daemon..." | ts '%Y-%m-%d %H:%M:%.S'
if [ -z "${PROXY_PATH}" ]
then
    su $PUSERNAME -c "iptv-proxy --xtreme-base-url ${XTREME_BASE_URL} --xtreme-user ${XTREME_USERNAME} --xtreme-password ${XTREME_PASSWORD} --port $WEB_PORT --user $PROXY_USER --password $PROXY_PASS --hostname $PROXY_HOST &"
else
    echo "[info] Proxy path: ${PROXY_PATH}" | ts '%Y-%m-%d %H:%M:%.S'
    su $PUSERNAME -c "iptv-proxy --xtreme-base-url ${XTREME_BASE_URL} --xtreme-user ${XTREME_USERNAME} --xtreme-password ${XTREME_PASSWORD} --port $WEB_PORT --user $PROXY_USER --password $PROXY_PASS --hostname $PROXY_HOST --custom-endpoint $PROXY_PATH &"
fi
